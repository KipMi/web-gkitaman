...
const KegiatanPemudaPage = () => {
  ...
  const kegiatanPemuda = kegiatans.filter(
    (item) => item.komisi === 'Komisi Pemuda'
  );

  return (
    <div className="h-auto min-h-screen m-10 flex flex-wrap justify-between">
      {kegiatanPemuda.length > 0 ? (
        kegiatanPemuda.map((kegiatan: kegiatanType, i: number) => {
          return (
            <KegiatanCard
...
export default KegiatanPemudaPage;
